...
def _float_hex_to_int(f):
    ...
    if x == 0:
        r = (1 << half_len) + \
            ((1 << (len(h[x + 4:p]) * 4)) + int(h[x + 4:p], 16))
    else:
        r = (1 << half_len) - \
            ((1 << (len(h[x + 4:p]) * 4)) + int(h[x + 4:p], 16))
...
def decode(hashcode, delta=False):
    ...
    if hasattr(float, "fromhex"):
        ...
        longitude = _int_to_float_hex(
            lon, lon_length) * 180.0 + longitude_delta
        if delta:
            return latitude, longitude, latitude_delta, longitude_delta
        return latitude, longitude
...
def bbox(hashcode):
    ...
    ret = {}
    if lat_length:
        ret['n'] = 180.0 * (lat + 1 - (1 << (lat_length - 1))
                            ) / (1 << lat_length)
    ...
    if lon_length:
        ret['e'] = 360.0 * (lon + 1 - (1 << (lon_length - 1))
                            ) / (1 << lon_length)
...
def _uint64_interleave(lat32, lon32):
    ...
    for i in range(8):
        intr = (intr << 8) + (boost[(lon32 >> (28 - i * 4)) %
                                    16] << 1) + boost[(lat32 >> (28 - i * 4)) % 16]
